...
def index():
    ...
    return render_template('index.html', title=u'Główna', form=form,
...
def user(username):
    ...
    prev_url = url_for('user', username=user.username, page=posts.prev_num) \
        if posts.has_prev else None
    form = EmptyForm()
    return render_template('user.html', title="Profil", user=user,
                           posts=posts.items, next_url=next_url,
...
def edit_profile():
    ...
    return render_template('edit_profile.html', title="O mnie",
                           form=form)
...
def explore():
    ...
    prev_url = url_for('explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('explore.html', title='Odkrywaj', posts=posts.items,
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template('reset_password.html', title=u'Zresetuj', form=form)
